...
| 4 | `transponder_setting` relaxed from required to optional — structures without a warp-in transponder beacon omit it entirely. Pure relaxation; migration from v3 only bumps the version stamp. |
| 5 | `structure_type` and `transponder_setting` become discriminated unions (`z.discriminatedUnion`), combined via `z.intersection` so each axis narrows independently in TypeScript. `structure_type` is now a fixed enum (`gate` \| `catapult` \| `storage_unit` \| `turret`); `catapult` requires a new `destination_solar_system` field, `gate` deliberately has none (a gate's destination is derived from on-chain state, not stored here). `transponder_setting` gains an explicit `'none'` variant replacing v4's "field omitted" convention. Migration from v4: omitted `transponder_setting` becomes `'none'`; pre-existing `catapult` locations (which predate `destination_solar_system`) are backfilled with the `DESTINATION_UNKNOWN` sentinel and must be patched via `updateLocation`; an unrecognized `structure_type` throws `ValidationFailed`. |
