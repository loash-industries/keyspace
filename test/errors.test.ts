...
describe('AclClientError', () => {
  it('sets name, code, and message', () => {
    const err = new AclClientError(AclError.AccessDenied, 'access denied message');
  ...
  it('stores the cause when provided', () => {
    ...
    const err = new AclClientError(AclError.StorageUploadFailed, 'upload failed', rootCause);
  ...
  it('is catchable as an Error', () => {
    ...
    expect(() => { throw err; }).toThrow('not found');
  });
