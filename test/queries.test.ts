...
describe('fetchAdminCaps', () => {
  it('returns an empty array when no objects are returned', async () => {
    const client = makeSuiClient({ getOwnedObjects: (jest.fn() as any).mockResolvedValue({ data: [] }) });
        ...
        data: [
          {
            data: {
              objectId: '0xcap1',
              content: { dataType: 'moveObject', fields: { allowlist_id: ACL_ID } },
            },
          },
          {
            data: {
              objectId: '0xcap2',
              content: { dataType: 'moveObject', fields: { allowlist_id: '0xacl002' } },
            },
          },
        ],
      }),
...
describe('fetchAllowListMeta', () => {
  it('returns null when content is not moveObject', async () => {
    const client = makeSuiClient({
      getObject: (jest.fn() as any).mockResolvedValue({ data: { content: { dataType: 'package' } } }),
    });
...
describe('fetchAccessibleAcls', () => {
  ...
  it('returns aclIds from the indexer', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(`${INDEXER}/v1/address/${OWNER}/acls`);
  });
  ...
  it('throws AclClientError(UnexpectedResponse) on non-ok response', async () => {
    ...
    await expect(fetchAccessibleAcls(INDEXER, OWNER)).rejects.toThrow(AclClientError);
