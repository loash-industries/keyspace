...
export class AclClient {
  ...
  async createAcl(opts: {
    ...
    tx.moveCall({
      ...
      arguments: [tx.pure.vector('u8', Array.from(new TextEncoder().encode(opts.name)))],
    });
  ...
  async getAcl(aclId: string): Promise<AclDetail> {
    const detail = await fetchAllowListDetail(this.suiClient, this.packageId, aclId);
    if (!detail) {
      throw new AclClientError(AclError.EntryNotFound, `ACL not found: ${aclId}`);
    }
    return detail;
  }
  ...
  async removeRole(opts: {
    ...
    const grantee = roleToAddress(opts.role);
    const tx = removeMemberTx(this.packageId, opts.aclId, opts.adminCapId, grantee);
    await this.executor(tx);
    const meta = await this.getAclMeta(opts.aclId);
    return { epoch: meta.epoch };
  }
  ...
  async resolveRoles(opts: { aclId: string; address: string }): Promise<Role[]> {
    const acl = await this.getAcl(opts.aclId);
  ...
  async writeData(opts: {
    ...
    const tx = publishEntryTx(this.packageId, opts.aclId, location, opts.description);
    const result = await this.executor(tx);
  ...
  async readData(opts: {
    ...
    const meta = await this.getAclMeta(opts.aclId);
    const entry = await fetchEncryptedEntry(this.suiClient, opts.entryId, meta.epoch);
    if (!entry) {
      throw new AclClientError(AclError.EntryNotFound, `Entry not found: ${opts.entryId}`);
    }
  ...
  async rotateEntry(opts: {
    ...
    const tx = updateEntryTx(this.packageId, opts.aclId, opts.entryId, newLocation);
    await this.executor(tx);
  ...
  async transferAdminCap(opts: { adminCapId: string; newOwner: string }): Promise<void> {
